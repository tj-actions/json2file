...
if [[ -z "$INPUT_BIN_PATH" ]]; then
  ...
  if [[ "$ARCHIVE" == "zip" ]]; then
    curl -sf https://github.com/tj-actions/json2file/releases/download/"$LATEST_VERSION"/json2file_"$LATEST_VERSION"_"$TARGET"."$ARCHIVE" -o "$TEMP_DIR"/json2file.zip
    unzip -q "$TEMP_DIR"/json2file.zip -d "$TEMP_DIR"
    chmod +x "$TEMP_DIR"/json2file
  else
    curl -sf https://github.com/tj-actions/json2file/releases/download/"$LATEST_VERSION"/json2file_"$LATEST_VERSION"_"$TARGET"."$ARCHIVE" -o "$TEMP_DIR"/json2file.tar.gz
